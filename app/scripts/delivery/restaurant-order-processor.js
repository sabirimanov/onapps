(function() {
	...
	function restaurantOrderProcessor($filter, $cordovaEmailComposer, _, $q) {
		...
		function sendDineInConfirmation(cart, restaurant, destination) {
			var confirmation = 'Table: ' + destination.table + '<br/>';
			...
			return sendEmail(restaurant.email, subject, confirmation);
		}

		function sendTakeAwayConfirmation(cart, restaurant, destination) {
			var confirmation = 'Hi ' + destination.fullname + '<br/>';
			if (destination.phone) {
				confirmation += 'Your phone number ' + destination.phone + '<br/><br/>';
			}
